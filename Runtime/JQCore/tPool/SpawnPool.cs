...
namespace JQCore.tPool
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        {
            var message = "Use IsSpawned(Transform instance) instead.";
        ...
        public bool _dontDestroyOnLoad; // Property backer and used by GUI.

        /// <summary>
        ///     A list of PreloadDef options objects used by the inspector for user input
        /// </summary>
        public List<PrefabPool> _perPrefabPoolOptions = new();
        ...
        /// </summary>
        public Dictionary<object, bool> prefabsFoldOutStates = new();
        ...
        public Dictionary<object, bool> _editorListItemStates = new();
        ...
        /// </summary>
        public PrefabsDict prefabs = new();
        ...
        public Dictionary<string, PrefabPool> prefabPools
        {
            get
            {
                ...
                for (var i = 0; i < _prefabPools.Count; i++)
        ...
        private readonly List<PrefabPool> _prefabPools = new();
        internal List<Transform> _spawned = new();
        ...
        private void Awake()
        {
            ...
            for (var i = 0; i < _perPrefabPoolOptions.Count; i++)
            {
            ...
            //   possibility of adding a badly initUI pool.
        ...
        private void OnDestroy()
        {
            ...
            _spawned.Clear();
            // Clear-up
            foreach (var pool in _prefabPools) pool.SelfDestruct();
        ...
        public void CreatePrefabPool(PrefabPool prefabPool)
        {
            ...
            //   second in cases where a user-script beats out PoolManager's initUI during 
            //   Awake();
            var isAlreadyPool = GetPrefabPool(prefabPool.prefab) == null ? false : true;
            if (!isAlreadyPool)
            {
        ...
        public void RemovePrefabPool(PrefabPool prefabPool)
        {
            ...
            if (isAlreadyPool)
            {
                ...
                // Add to the prefabs dict for convenience
                prefabs._Remove(prefabPool.prefab.name);
            }
        }
        ...
        public void Add(Transform instance, string prefabName, bool despawn, bool parent)
        {
            for (var i = 0; i < _prefabPools.Count; i++)
            {
                ...
                if (_prefabPools[i].prefabGO.name == prefabName)
                {
                    ...
                    if (parent) instance.parent = group;
            ...
            // Log an errorCode if a PrefabPool with the given name was not found
        ...
        {
            var msg = "Use SpawnPool.Spawn() to properly add items to the pool.";
        ...
        {
            var msg = "Use Despawn() to properly manage items that should " +
        ...
        ///     uses and returns a Transform reference (or other supported _type, such
        ...
        public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot, Transform parent)
        {
            ...
            var newPrefabPool = new PrefabPool(prefab);
        ...
        {
            var inst = Spawn(prefab, pos, rot, null);
            ...
            Transform parent)
        {
            ...
            var inst = Spawn(prefab.transform, pos, rot, parent);
        ...
        public PrefabPool GetPrefabPool(Transform prefab)
        {
            for (var i = 0; i < _prefabPools.Count; i++)
            {
                var tempPool = _prefabPools[i];
                if (tempPool.prefabGO == null)
                    Debug.LogError(string.Format("SpawnPool {0}: PrefabPool.prefabGO is null",
                        poolName));

                if (tempPool.prefabGO.name == prefab.gameObject.name) return tempPool;

                if (tempPool.prefabGO == prefab.gameObject)
                    return tempPool;
            }
    ...
    public class PrefabPool
    {
        ...
        ///     here will be used
        /// </summary>
        public bool _logMessages; // Used by the inspector
        ...
        /// </summary>
        public bool limitFIFO; // Keep after limitAmount for auto-inspector
        ...
        public bool logMessages // Read-only
        {
            get
            {
                ...
                if (spawnPool == null) return false;
        ...
        /// <description>
        ///     A pseudo constructor to initUI stuff not initUI by the serialized inspector-created
        ...
        {
            //HyDebug.LogError("SelfDestruce:"+this.prefabGO.name);
            ...
            foreach (var inst in _despawned)
                if (inst != null)
                    Object.Destroy(inst.gameObject);
            //HyDebug.LogError("                  remove:" + inst.rootGameObject.name);

            foreach (var inst in _spawned)
                if (inst != null)
                    Object.Destroy(inst.gameObject);
            //HyDebug.LogError("                  remove:" + inst.rootGameObject.name);
            _spawned.Clear();
        ...
        /// </summary>
        internal List<Transform> _despawned = new();
        ...
        public List<Transform> spawned => new(_spawned);

        public List<Transform> despawned => new(_despawned);
        ...
        internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
        {
            ...
            if (_despawned.Count == 0)
            {
                //HyDebug.Log("pool empty new another:" + prefab.name);
            ...
            else
            {
                ...
                // This came up for a user so this was added to throw a user-friendly errorCode
                if (inst == null)
                {
        ...
        private Action<string> _newAction;
        ...
        public Transform SpawnNew(Vector3 pos, Quaternion rot)
        {
            ...
            if (_newAction != null) _newAction(prefab.name);
            ...
            {
                if (inst is RectTransform)
                    inst.SetParent(spawnPool.group); // The group is the parent by default   
                else
                    inst.parent = spawnPool.group; // The group is the parent by default   
            }
        ...
        internal void PreloadInstances()
        {
            ...
            else
            {
                ...
                //HyDebug.Log("Instantiate:" + prefab.name + " num:" + preloadAmount);
        ...
        private IEnumerator PreloadOverTime()
        {
            ...
            var amount = preloadAmount - totalCount;
            if (amount <= 0)
                yield break;
            ...
            var remainder = amount % preloadFrames;
            var numPerFrame = amount / preloadFrames;

            // Reduce debug spam: Turn off this.logMessages then set it back when done.
            forceLoggingSilent = true;

            int numThisFrame;
            for (var i = 0; i < preloadFrames; i++)
            {
                // Add the remainder to the *last* frame
                numThisFrame = numPerFrame;
                if (i == preloadFrames - 1) numThisFrame += remainder;

                for (var n = 0; n < numThisFrame; n++)
                {
                    // Preload...
                    // This will parent, position and orient the instance
                    //   under the SpawnPool.group
    ...
    public class PrefabsDict : IDictionary<string, Transform>
    {
        ...
        private readonly Dictionary<string, Transform> _prefabs = new();
        ...
        {
            var msg = "Use Contains(string prefabName) instead.";
            throw new NotImplementedException(msg);
        }
        ...
        public Transform this[string key]
        {
            get
            {
                ...
                {
                    var msg = string.Format("A Prefab with the name '{0}' not found. " +
                                            "\nPrefabs={1}",
                        key, ToString());
