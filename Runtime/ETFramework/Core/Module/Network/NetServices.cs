...
namespace ET
{
    ...
    public class NetServices: Singleton<NetServices>
    {
        ...
        private readonly Dictionary<int, Action<long>> recvByteCallback = new Dictionary<int, Action<long>>();

        private int serviceIdGenerator;

        public (uint, uint) GetChannelConnInfo(int serviceId, long channelId)
        {
        ...
        public void RegisterRecvByteCallback(int serviceId, Action<long> action)
        {
            this.recvByteCallback.Add(serviceId, action);
        }
            ...
            while (true)
            {
                ...
                try
                {
                    switch (op.Op)
                    {
                        ...
                        case NetOp.OnRecvByte:
                        {
                            if (!this.recvByteCallback.TryGetValue(op.ServiceId, out var action))
                            {
                                return;
                            }

                            action.Invoke(op.ChannelId);
                            break;
                        }
                        case NetOp.OnError:
                        {
        ...
        public void OnRecvBytes(int serviceId, long channelId)
        {
            NetOperator netOperator = new NetOperator() { Op = NetOp.OnRecvByte, ServiceId = serviceId, ChannelId = channelId };
            this.mainThreadOperators.Enqueue(netOperator);
        }
        ...
        //NetClient专用
        public void RegisterChannelAddress(int serviceId, long channelId, IPEndPoint realIpEndPoint)
        {
            AService service = this.Get(serviceId);
            if (service == null)
            {
            ...
            service.RegisterChannelAddress(channelId, realIpEndPoint);
        }
    }
}