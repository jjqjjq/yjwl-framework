...
namespace ET
{
    public class ETJsonObject
    {
#if POOLING
	const int MAX_POOL_SIZE = 10000;
	public static Queue<ETJsonObject> releaseQueue = new Queue<ETJsonObject>();
#endif
	...
	public float f {
        ...
        public delegate void AddJSONConents(ETJsonObject self);

        public static ETJsonObject nullJO
        {
            get { return Create(Type.NULL); }
        } //an empty, null object

        public static ETJsonObject obj
        {
            get { return Create(Type.OBJECT); }
        } //an empty object

        public static ETJsonObject arr
        {
            get { return Create(Type.ARRAY); }
        } //an empty array

        public ETJsonObject(Type t)
        {
            type = t;
            switch (t)
            {
                case Type.ARRAY:
                    list = new List<ETJsonObject>();
                    break;
                case Type.OBJECT:
                    list = new List<ETJsonObject>();
        ...
        public ETJsonObject(bool b)
        {
            type = Type.BOOL;
            this.b = b;
        }
#if USEFLOAT
        public ETJsonObject(float f)
        {
            type = Type.NUMBER;
            n = f;
        }
#else
	public ETJsonObject(double d) {
		_type = Type.NUMBER;
		n = d;
	}
#endif
        public ETJsonObject(Dictionary<string, string> dic)
        {
            type = Type.OBJECT;
            keys = new List<string>();
            list = new List<ETJsonObject>();
        ...
        public ETJsonObject(Dictionary<string, ETJsonObject> dic)
        {
            type = Type.OBJECT;
            keys = new List<string>();
            list = new List<ETJsonObject>();
            //Not sure if it's worth removing the foreach here
            foreach (KeyValuePair<string, ETJsonObject> kvp in dic)
            {
                keys.Add(kvp.Key);
                list.Add(kvp.Value);
            }
        ...
        public ETJsonObject(AddJSONConents content)
        {
            content.Invoke(this);
        }

        public ETJsonObject(ETJsonObject[] objs)
        {
            type = Type.ARRAY;
            list = new List<ETJsonObject>(objs);
        }

        //Convenience function for creating a ETJsonObject containing a string.  This is not part of the constructor so that malformed JSON data doesn't just turn into a string object
        public static ETJsonObject StringObject(string val)
        {
            return CreateStringObject(val);
        }

        public void Absorb(ETJsonObject obj)
        {
        ...
        public static ETJsonObject Create()
        {
#if POOLING
		ETJsonObject result = null;
		...
		if(result != null)
			return result;
#endif
            return new ETJsonObject();
        }

        public static ETJsonObject Create(Type t)
        {
            ETJsonObject obj = Create();
            obj.type = t;
            switch (t)
            {
                case Type.ARRAY:
                    obj.list = new List<ETJsonObject>();
                    break;
                case Type.OBJECT:
                    obj.list = new List<ETJsonObject>();
        ...
        public static ETJsonObject Create(bool val)
        {
            ETJsonObject obj = Create();
            obj.type = Type.BOOL;
            obj.b = val;
        ...
        public static ETJsonObject Create(float val)
        {
            ETJsonObject obj = Create();
        ...
        public static ETJsonObject Create(int val)
        {
            ETJsonObject obj = Create();
            obj.type = Type.NUMBER;
            obj.n = val;
        ...
        public static ETJsonObject CreateStringObject(string val)
        {
            ETJsonObject obj = Create();
        ...
        public static ETJsonObject CreateBakedObject(string val)
        {
            ETJsonObject bakedObject = Create();
        ...
        /// <summary>
        /// Create a ETJsonObject by parsing string data
        /// </summary>
        ...
        /// <param name="storeExcessLevels">Whether to store levels beyond maxDepth in baked ETJsonObjects</param>
        /// <param name="strict">Whether to be strict in the parsing. For example, non-strict parsing will successfully 
        /// parse "a string" into a string-_type </param>
        /// <returns></returns>
        public static ETJsonObject Create(string val, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false)
        {
            ETJsonObject obj = Create();
        ...
        public static ETJsonObject Create(AddJSONConents content)
        {
            ETJsonObject obj = Create();
            content.Invoke(obj);
            return obj;
        ...
        public static ETJsonObject Create(Dictionary<string, string> dic)
        {
            ETJsonObject obj = Create();
        ...
        public ETJsonObject(string str, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false)
        {
            //create a new ETJsonObject from a string (this will also create any children, and parse the whole string)
        ...
        void Parse(string str, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false)
        {
            if (!string.IsNullOrEmpty(str))
            {
                ...
                if (strict)
                {
                    ...
                    {
                        type = Type.NULL;
                        Log.Warning("Improper (strict) JSON formatting.  First character must be [ or {");
                        return;
                    }
                }
        ...
        public void Add(ETJsonObject obj)
        {
            if (obj)
            {
        ...
        public void AddField(string name, ETJsonObject obj)
        {
            if (obj)
            {
                //Don't do anything if the object is null
        ...
        public void SetField(string name, ETJsonObject obj)
        {
            if (HasField(name))
            {
        ...
        public delegate void GetFieldResponse(ETJsonObject obj);
	...
	public bool GetField(ref double field, string name, FieldNotFound fail = null) {
        ...
        public ETJsonObject GetField(string name)
        {
            if (IsObject)
        ...
        /// <summary>
        /// Copy a ETJsonObject. This could probably work better
        /// </summary>
        /// <returns></returns>
        public ETJsonObject Copy()
        {
            return Create(Print());
        }
         ...
         */
        public void Merge(ETJsonObject obj)
        {
            MergeRecur(this, obj);
        }
        ...
        static void MergeRecur(ETJsonObject left, ETJsonObject right)
        {
            ...
            else if (left.type == Type.ARRAY && right.type == Type.ARRAY)
            {
                ...
                {
                    Log.Error("Cannot merge arrays when right object has more elements");
                    return;
                }
                ...
                for (int i = 0; i < right.list.Count; i++)
                {
                    ...
                    {
                        //Only overwrite with the same _type
...
#pragma warning restore 219
        ...
        {
            //Convert the ETJsonObject into a string
        ...
        public ETJsonObject this[int index]
        {
            get
            {
        ...
        public ETJsonObject this[string index]
        {
        ...
        public Dictionary<string, string> ToDictionary()
        {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    switch (val.type)
                    {
                        ...
                        case Type.BOOL:
                            ...
                            break;
                        default:
                            Log.Warning("Omitting object: " + keys[i] + " in dictionary conversion");
                            break;
                    }
                }
            ...
            Log.Warning("Tried to turn non-Object ETJsonObject into a dictionary");
            return null;
        }

        public static implicit operator bool(ETJsonObject o)
        {
            return o != null;
        }
#if POOLING
