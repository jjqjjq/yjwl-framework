...
namespace HardCodeLab.TutorialMaster
{
    ...
    public abstract class Module : MonoBehaviour
    {
        ...
        public void Activate(TutorialMasterManager caller, ModuleSettings settings, string languageKey = "")
        {
            if (!InUse)
            {
                TMLogger.LogInfo(string.Format("Activating {0} of name \"{1}\"...", GetType().Name, gameObject.name),
                    caller);

                _currentSettings = settings;

                if (NullChecker.IsNull(_currentSettings.TargetCanvas,
                        "TargetCanvas has not been assigned! Aborting activation...", caller))
                    return;
            ...
            {
                TMLogger.LogInfo(string.Format(
                    "Module of the name \"{0}\" is already in use by Tutorial Master GameObject named \"{1}\". Ignoring...",
        ...
        public virtual void SetLanguage(string languageKey)
        {
            ...
            TMLogger.LogInfo(string.Format("Applying language changes for this module. Key: {0}", languageKey),
                CallerManager);
        ...
        {
            TMLogger.LogInfo(string.Format(
                "Calculating a position based on placement of type \"{0}\" based on a RectTransform of {1}",
        ...
        {
            TMLogger.LogInfo(
                string.Format("Converting a position of {0} in order to position it within the canvas space...",
                    position), CallerManager);
            ...
            TMLogger.LogInfo(string.Format("Canvas-space position has been calculated! It's {0}", newPosition),
                CallerManager);
        ...
        {
            ModulePosition =
                RectTransformUtility.PixelAdjustPoint(RectTransform.localPosition, RectTransform, CurrentCanvas);
        }
        ...
        private Vector2 ConstrainToCanvas(Vector2 pos, Borders offset)
        {
            ...
            var canvasSize = RectTransformUtility
                .PixelAdjustRect(RectTransform.parent.GetComponent<RectTransform>(), CurrentCanvas).size;
        ...
        private void UpdateParent()
        {
            ...
            if (NullChecker.IsNull(_currentSettings.TargetCanvas,
                    "Unable to assign a parent! TargetCanvas has not been assigned.", CallerManager))
                return;

            var newParent = _currentSettings.TargetCanvas.transform;

            if (NullChecker.IsNull(RectTransform, "RectTransform is null! Unable to assign a parent. Ignoring...",
                    CallerManager))
                return;
